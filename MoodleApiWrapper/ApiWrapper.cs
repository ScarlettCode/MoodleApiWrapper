...
namespace MoodleApiWrapper
{
    public class ApiWrapper
    {
        ...
        /// <returns></returns>
        public static Task<ApiResponse<Site_info>> GetSiteInfo( Methods serviceHostNames = Methods.default_)
        {
            if(HostIsSet && TokenIsSet)
            {

                string query = string.Empty;
                if (serviceHostNames == Methods.default_)
                {
                ...
                else
                {
                    query = string.Format("webservice/rest/server.php" +
                         ...
                         ApiToken, ParseFormat(Format.JSON), ParseMethod(Methods.core_webservice_get_site_info), ParseMethod(serviceHostNames));
